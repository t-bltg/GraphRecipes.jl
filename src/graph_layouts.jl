...
function spring_graph(adjmat::AbstractMatrix;
                      dim = 2,
                      x = rand(Graphs.getRNG(), size(adjmat)[1]),
                      y = rand(Graphs.getRNG(), size(adjmat)[1]),
                      z = rand(Graphs.getRNG(), size(adjmat)[1]),
...
#                               x = rand(Graphs.getRNG(), length(node_weights)),
#                               y = rand(Graphs.getRNG(), length(node_weights)),
#                               z = rand(Graphs.getRNG(), length(node_weights)))
#     adjmat = make_symmetric(adjmat)
...
function tree_graph(source::AbstractVector{Int}, destiny::AbstractVector{Int}, weights::AbstractVector;
    ...
    if layers == nothing
        # layers = rand(Graphs.getRNG(), 1:4, n)
    ...
    # add noise
    if add_noise
        layers = layers + 0.6rand(Graphs.getRNG(), size(layers)...)
    end
